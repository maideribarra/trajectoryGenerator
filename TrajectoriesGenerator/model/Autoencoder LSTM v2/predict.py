...
if __name__ == "__main__":
    ...
    numEjemplo=50
    tsinput=npres[0].movedim(0,1)
    tsoutput=npres[1].movedim(0,1)
    ...
    plt.show()
    #plt.savefig(dir+numEjemplo)
    outputs=[]  
    out = predictions[0][0][1].movedim(0,1)
    plt.plot(out[0][:][:,0],out[0][:][:,1])
    #plt.plot(tsoutput[1][:][:,0],tsoutput[1][:][:,1]) 
    plt.show()
    contador=0
    for batch in predictions:
        output=batch[0][1].movedim(0,1)
        for traj in output:
    ...
    print(contador)
    # plt.show()
    print('size outputs', len(outputs))
    print('size outputs [0]', len(outputs[0]))
    print(type(outputs[0]))
    npOutput=np.asarray(outputs)
    print('size outputs', npOutput.shape)
    npOutputDiff=np.unique(npOutput, axis=0)
    print('size unique outputs', npOutputDiff.shape)
